...
var _ = Describe("Test CRUD of PlacementRuleClient", func() {
	...
	err := pkg.LoadYamlFileToK8sTypedObject(testdataDir+"/placement-rule.sample.yaml", &typedObj)
	Expect(err).NotTo(HaveOccurred())

	var clientInterface dynamic.NamespaceableResourceInterface
	var client placementRuleClient

	BeforeEach(func() {
		clientInterface = ocmK8ResourceInterfaceSet.PlacementRule
		client = NewPlacementRuleClient(clientInterface)
	})

	Context("When creating PlacementRule", func() {
	...
	Context("When deleting PlacementRule", func() {
